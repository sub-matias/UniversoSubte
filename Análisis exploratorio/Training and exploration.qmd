---
title: "Training and exploration"
author: "Matias S"
format: html
editor: visual
---

```{r setup, echo=FALSE}
library(tidyverse, quietly = TRUE)
library(ggthemes)
theme_set(
  theme_test()
)
library(googlesheets4)
library(kableExtra)
library(RColorBrewer)
```

#### Acceso a los datos

```{r source-del-script, echo=FALSE}
gs4_deauth()

my_url <- "(https://docs.google.com/spreadsheets/d/1SF5DDTEfbQFSNjvYKQnsQKdsBdE7oJeTywCk92pB7f8/edit#gid=1964221805)"

subte <- googlesheets4::read_sheet(my_url)
subte <- subte %>%
  mutate(Year = as.integer(Periodo)) %>%
  mutate(Periodo = as.factor(Periodo))
```

#### Propósito

Documento que aborda la exploración de los datos sobre el Subte Publicitario en su primera versión: totalidad de publicidades encontradas sin discriminación de línea de subterráneo, año y dispositivo.

```{r, echo=FALSE}
subte %>%
  knitr::kable(caption = "Raw Data")%>%
  kable_paper()%>%
  kable_styling(bootstrap_options = "striped", font_size = 12)
```

#### Análisis exploratorio

#### Años

Cantidad de publicidades con presencia efectiva en el sistema subterráneo dentro de la base de datos:

```{r, echo=FALSE}
#Timelines
subte %>%
  group_by(Year) %>%
  tally() %>%
  ggplot(aes(x=Year,y=n))+
  geom_line()+
  geom_point(size=3)+
  geom_point(size=2,color="white")+
  xlab("Año")+
  ylab("Publicidades")+
  ggtitle("Distribución temporal")
```

```{r, echo=FALSE}
subte %>% 
  group_by(Periodo) %>% 
  tally() %>% 
  knitr::kable(
    caption = "Numero de publicidades por año",
    col.names = c("Año", "Número observado")
               )
```

Es factible observar que la primera etapa de la recolección de publicidades está relacionada con la proximidad temporal, en la cual existe mayor probabilidad de encontrar "buenos datos" en años cercanos al 2022.

#### Líneas de subterráneo

Distribución de las publicidades en relación a la línea de subterráneo en la cual han manifestado su presencia

```{r, echo=FALSE}
#add hex
subte %>%
  group_by(Linea, Periodo) %>%
  tally() %>%
  ggplot(aes(x=Periodo,y=n,fill=Linea))+
  geom_col()+
   scale_fill_manual(values = c(A="#339BBB",B="#C63C3C",D="#359041",H="#F8C301"))+
  xlab("Años")+
  ylab("Publicidades")+
  ggtitle("Distribución subterránea")
```

Es apreciable la abundancia de material posible de recolectar por sobre la Línea B de Subte.

#### RQ1: ¿Cómo es representado el tiempo en las publicidades gráficas del Subte?

Dados los parámetros arbitrarios de la base de datos, es interés abordar la variable de `orientación`

```{r, echo=FALSE}
subte$Orientacion %>%
  unique()
```

```{r, echo=FALSE}
subte %>%
  group_by(Orientacion) %>%
  tally() %>%
  ggplot(aes(x=n, y=Orientacion))+
  geom_col(fill="#4D5382")+
  xlab("Publicidades")+
  ylab("Orientación")+
  ggtitle("Distribución de representación temporal")
```

Si bien la cantidad de publicidades en `subte` es escasa, verifica una hipótesis vista en los antecedentes teóricos de la tesina acerca de la actividad publicitaria occidental:

💡 Existe la tendencia publicitaria a referirse al presente, aunque existen aquellas que refieren al futuro o al pasado.

```{r, echo=FALSE}
subte %>%
  group_by(Orientacion, Producto) %>%
  tally() %>%
  ggplot(aes(x=Producto,y=Orientacion))+
  geom_raster(fill="#4D5382")+
  ggtitle("Uso del tiempo según producto")
```

#### RQ2:¿Cuál es la utilización de la paleta de colores en las publicidades de la Ciudad de Buenos Aires?

Distribución de escala cromática utilizada por las publicidades en razón de la categoría `Productos`

```{r, echo=FALSE}
subte %>%
  group_by(Cromatica, Producto) %>%
  tally() %>%
  ggplot(aes(x=Producto,y=n,fill=Cromatica))+
  geom_col()+
  scale_fill_manual(values = c(Frío="#658e96",Cálido="#fe6d73"))+
  ggtitle("Escala")+
  ylab("Publicidades")+
  xlab("Producto comercializado")
```

#### RQ3: HEX Publicitario

Ensayo de representación:

```{r, echo=FALSE}
display.brewer.pal(n = 8, name = 'RdBu')
```
